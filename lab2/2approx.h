#ifndef LAB2_2APPROX_H_
#define LAB2_2APPROX_H_
...
two_approx_backpack(const std::vector<Item> &data, const size_t capacity) {
    ...
    size_t remain_capacity =
            capacity - current_weight, max_index = index.size();
    for (; i < index.size(); ++i) {
        ++iter;
        if (data[index[i]].weight <= remain_capacity) {
            if (max_index == index.size() || (max_index != index.size() && (
                    static_cast<double>(data[index[i]].price) /
                    static_cast<double>(data[index[i]].weight) >
                    static_cast<double>(data[index[max_index]].price) /
                    static_cast<double>(data[index[max_index]].weight)))) {
                max_index = i;
            }
        }
    }
